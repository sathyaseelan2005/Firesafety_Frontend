        ...
        <AnimatePresence>
            {isVisible && (
                <motion.div
                    ...
                    className={`${className} absolute -top-8 left-1/2 w-fit whitespace-nowrap rounded-md border border-neutral-700 bg-[#060010] px-2 py-1 text-xs text-white z-50`}
                    role="tooltip"
                    style={{ x: '-50%' }}
                >
                    {children}
    ...
    return (
        <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center z-50">
            <motion.div
                ...
                className={`${className} absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-4 rounded-2xl border-neutral-700 border-2 pb-2 px-4 bg-[#060010]/80 backdrop-blur-md`}
